...
```{r ans11}
# loss1Companies
lossCompanies <- finStatement[["營業利益（損失）"]] <=0
map(finStatement,~.[lossCompanies]) -> loss1Companies
loss1Companies <- loss1Companies[["公司名稱"]]
loss1Companies
```
...
```{r ans12}
# loss2Companies
x <- finStatement[["基本每股盈餘（元）"]] <=0
map(finStatement,~.[x]) -> loss2Companies
loss2Companies <- loss2Companies[["公司名稱"]]
loss2Companies
```
...
```{r ans13}
# only1LossCompanies
gainprofit <- finStatement[["營業利益（損失）"]] >=0
map(finStatement,~.[gainprofit]) -> gainprofit01
gainprofit01 <- gainprofit01[["公司名稱"]]

gain  <- finStatement[["基本每股盈餘（元）"]] >=0
map(finStatement,~.[gain]) -> gain01
gain01 <- gain01[["公司名稱"]]
only1LossCompanies <- c_((gainprofit01 & loss2Companies) or (gain01 & loss1Companies))
only1LossCompanies
...
# df_oneShow
qq <- exhibitionList[[1]]
df_oneShow <- data.frame(qq$.title, qq$.showInfo[["time"]],qq$.showInfo[["location"]],
                         qq$.showInfo[["locationName"]],
                         qq$.showInfo[["price"]],
                         qq$.showInfo[["endTime"]],
                         qq$.showInfo[["sourceWebName"]])
df_oneShow
```
...
```{r ans25}
# totalCount
totalCount <- sum(exhibitionList[["sourceWebName"]] == "全國藝文活動資訊系統")
totalCount
```
